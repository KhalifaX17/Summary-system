...
```mermaid
flowchart TD
    A[เจ้าหน้าที่สาขา ICT] -->|เปิดลิงก์เว็บแอป| B[Next.js Web App บน Vercel]
    B --> C{ต้องการทำอะไร}

    C -->|ดูข้อมูล| D[รายการโครงการ / แดชบอร์ด]
    D -->|อ่านข้อมูล| F[(Google Sheet<br/>ผ่าน Service Account)]

    C -->|เพิ่ม / แก้ไข| E[ฟอร์มบันทึกโครงการ]
    E -->|Server Action: บันทึก| F
    E -->|แนบไฟล์หลักฐาน| G[(Google Drive)]

    F --> H[แดชบอร์ดคำนวณสรุปใหม่อัตโนมัติ]
    H --> I{เลือกส่งออกรายงาน}
    I --> J[Excel .xlsx]
    I --> K[PDF]
    I --> L[CSV]
```
